...
-- This script will preform a control surface manipulation
-- The desired axis to perform a manipulation on is selected and started with a momentary switch
-- The plane maintains trim conditions while the switch is activated using servo overrides
-- If the momentary switch is released before "[MANIPULATION] FINISHED" is seen on the GCS,
-- the aircraft recovers to FBWA mode to assist the pilot.
-- Magnitude and duration of the manipulation can also be controlled.
-- It is suggested to allow the aircraft to trim for straight, level, unaccelerated flight (SLUF) in FBWB mode before
-- starting a manipulation
-- Original Creator: Charlie Johnson, Oklahoma State University 2020
-- Modified By: Benjamin Durante, University of Calgary 2021

-- This code has been modified for the elevon aircrafts used in the AeroCore lab
...
if UAV_AIRFRAME == 1 then
    ACTUATION_RANGE_LIMIT_ELEVONS = 28 -- range of motion of deflection
...
elseif UAV_AIRFRAME == 2 then
    ACTUATION_RANGE_LIMIT_ELEVONS = 49
...
function identification()
    ...
    if arming:is_armed() == true and rc:get_pwm(DOUBLET_ACTION_CHANNEL) > 1700 and end_time ==-1 then
        ...
        if rc:get_pwm(MANEUVER_ACTION_CHANNEL) > 1300 and rc:get_pwm(MANEUVER_ACTION_CHANNEL) < 1700 then -- ramp input
            DOUBLET_TIME = DOUBLET_TIME_ORIGINAL * 2
            callback_time = 10 --shorter callback time for a ramp input
        else
        ...
        if start_time == -1 then
            ...
            ramp_start_time = tonumber(tostring(now)) -- saving start time of doublet to compute the ramp rate
            if rc:get_pwm(MANEUVER_ACTION_CHANNEL) < 1300 then
                -- notify the gcs that we are starting a doublet
                gcs:send_text(6, "STARTING DOUBLET " .. DOUBLET_FUNCTION1)
            elseif rc:get_pwm(MANEUVER_ACTION_CHANNEL) > 1300 and rc:get_pwm(MANEUVER_ACTION_CHANNEL) < 1700 then
                -- notify the gcs that we are starting a doublet
                gcs:send_text(6, "STARTING RAMP DOUBLET " .. DOUBLET_FUNCTION1)
            elseif rc:get_pwm(MANEUVER_ACTION_CHANNEL) > 1700 then
                -- notify the gcs that we are starting a doublet
        ...
        elseif rc:get_pwm(MANEUVER_ACTION_CHANNEL) > 1300 and rc:get_pwm(MANEUVER_ACTION_CHANNEL) < 1700 then
            if ACTIVE_AILERON == true or ACTIVE_ELEVATOR == true then
                ...
                if now < start_time + (DOUBLET_TIME * 1/6) then
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 1/6) - ramp_start_time))
                    up = doublet_srv_trim2 + math.floor((doublet_srv_max2 - doublet_srv_trim2) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * opposite_elevon_motion * (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 1/6) - ramp_start_time))
               ...
               elseif now < start_time + (DOUBLET_TIME * 3/6) then
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * (1 - (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 3/6) - ramp_start_time)))
                    up = doublet_srv_trim2 + math.floor((doublet_srv_max2 - doublet_srv_trim2) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * opposite_elevon_motion * (1 - (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 3/6) - ramp_start_time)))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 4/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * ((tonumber(tostring(now)) - (ramp_start_time + DOUBLET_TIME * 1/6)) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 4/6) - (ramp_start_time + DOUBLET_TIME * 1/6))))
                    down = doublet_srv_trim2 - math.floor((doublet_srv_trim2 - doublet_srv_min2) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * opposite_elevon_motion * ((tonumber(tostring(now)) - (ramp_start_time + DOUBLET_TIME * 1/6)) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 4/6) - (ramp_start_time + DOUBLET_TIME * 1/6))))
                ...
                elseif now < start_time + (DOUBLET_TIME * 6/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 6/6) - ramp_start_time))))
                    down = doublet_srv_trim2 - math.floor((doublet_srv_trim2 - doublet_srv_min2) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_ELEVONS) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 6/6) - ramp_start_time))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif (now > (start_time + DOUBLET_TIME)) and (now < (start_time + DOUBLET_TIME + callback_time)) then
                    -- notify GCS
                    gcs:send_text(6, "RAMP DOUBLET FINISHED")
                    -- stick fixed at pre doublet trim position
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, doublet_srv_trim1, DOUBLET_TIME * (OBSERVATION_TIME - 1))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, doublet_srv_trim2, DOUBLET_TIME * (OBSERVATION_TIME - 1))
                elseif (now > start_time + DOUBLET_TIME + callback_time) and (now < start_time + (DOUBLET_TIME * OBSERVATION_TIME)) then
                ...
                elseif now > start_time + (DOUBLET_TIME * OBSERVATION_TIME) then
                    -- wait for RC input channel to go low
                    end_time = now
                    gcs:send_text(6, "RAMP DOUBLET OBSERVATION FINISHED")
                else
                    gcs:send_text(6, "this should not be reached")
                end
            elseif ACTIVE_RUDDER == true then
                -- split time evenly between high and low signal
                if now < start_time + (DOUBLET_TIME * 1/6) then
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER) * (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 1/6) - ramp_start_time))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 2/6) then 
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER))
                    ...
                    ramp_start_time = tonumber(tostring(now))
               elseif now < start_time + (DOUBLET_TIME * 3/6) then
                    up = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 3/6) - ramp_start_time))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 4/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER) * ((tonumber(tostring(now)) - (ramp_start_time + DOUBLET_TIME * 1/6)) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 4/6) - (ramp_start_time + DOUBLET_TIME * 1/6))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 5/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER))
                    ...
                    ramp_start_time = tonumber(tostring(now))
                elseif now < start_time + (DOUBLET_TIME * 6/6) then 
                    down = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_RUDDER) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 6/6) - ramp_start_time))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan2, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif (now > (start_time + DOUBLET_TIME)) and (now < (start_time + DOUBLET_TIME + callback_time)) then
                    -- notify GCS
                ...
                -- split time evenly between high and low signal
                if now < start_time + (DOUBLET_TIME * 1/6) then
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE) * (tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 1/6) - ramp_start_time))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 2/6) then 
                    down = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                    ramp_start_time = tonumber(tostring(now))
               elseif now < start_time + (DOUBLET_TIME * 3/6) then
                    up = doublet_srv_trim1 - math.floor((doublet_srv_trim1 - doublet_srv_min1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 3/6) - ramp_start_time))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 4/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE) * ((tonumber(tostring(now)) - (ramp_start_time + DOUBLET_TIME * 1/6)) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 4/6) - (ramp_start_time + DOUBLET_TIME * 1/6))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif now < start_time + (DOUBLET_TIME * 5/6) then
                    up = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, up, math.floor(DOUBLET_TIME * 1/6) + 100)
                    ramp_start_time = tonumber(tostring(now))
                elseif now < start_time + (DOUBLET_TIME * 6/6) then 
                    down = doublet_srv_trim1 + math.floor((doublet_srv_max1 - doublet_srv_trim1) * (DOUBLET_MAGNITUDE / ACTUATION_RANGE_LIMIT_THROTTLE) * (1 - ((tonumber(tostring(now)) - ramp_start_time) / (tonumber(tostring(start_time)) + (DOUBLET_TIME * 6/6) - ramp_start_time))))
                    SRV_Channels:set_output_pwm_chan_timeout(doublet_srv_chan1, down, math.floor(DOUBLET_TIME * 1/6) + 100)
                elseif (now > (start_time + DOUBLET_TIME)) and (now < (start_time + DOUBLET_TIME + callback_time)) then
                    -- notify GCS
                    gcs:send_text(6, "RAMP DOUBLET FINISHED")
    ...
    return identification, callback_time
end

gcs:send_text(6, "identification.lua is running")
return identification(), 1000